    ...
    fn rewrite_being(&self, mem: &CreatureMemory) -> std::io::Result<()> {
        ...
        if !mem.learned_facts.is_empty() {
            ...
            for f in mem.learned_facts.iter().rev().take(8) {
                // ⚡ Bolt optimization: Avoid hidden allocation for truncation.
                // Slice the original string directly using char boundaries.
                let mut iter = f.char_indices();
                let line = match iter.nth(200) {
                    Some((idx, _)) => &f[..idx],
                    None => f.as_str(),
                };
